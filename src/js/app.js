...
btnConfirma.addEventListener('click', () => {
    // Limpa o resultado antes de gerar a tabela
    resultado.innerHTML = '';
    ...
    enderecoHeader.textContent = 'Porcentagem';
    headerRow.appendChild(enderecoHeader);
    enderecoHeader.classList.add('centralizar'); // adiciona a classe para centralizar a coluna
    ...
    for (let loja of lojas) {
      ...
      enderecoLoja.textContent = loja.porc;
      row.appendChild(enderecoLoja);
      enderecoLoja.classList.add('centralizar'); // adiciona a classe para centralizar a coluna
  
      const total = document.createElement('td');
      loja.total = Math.ceil(valorDigitado.value * (loja.porc/100));
      total.textContent = loja.total.toFixed(0);
      row.appendChild(total);
      table.appendChild(row);
    }
